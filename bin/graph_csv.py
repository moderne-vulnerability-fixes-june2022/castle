...
parser.add_argument(
    '--utc', dest='use_utc', action='store_true',
    help="Use UTC for timestamps.")
parser.add_argument(
    '--time-is-milliseconds', dest='time_is_milliseconds', action='store_true',
    help="Interpret time as milliseconds.")
    ...
    for selector in column_selectors:
        ...
        if column is not None:
            ...
            for (timestamp, data) in column.values(cmd_args["shift_time_axis"],
                                                   derivative):
                if cmd_args["time_is_milliseconds"]:
                    timestamp = timestamp / 1000
                d = datetime.datetime.fromtimestamp(timestamp)
                if cmd_args["use_utc"]:
                    d = d.astimezone(UTC_TIME_ZONE)
                x.append(d)
                y.append(data)
